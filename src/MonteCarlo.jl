...
function MonteCarlo(
    ...
    overrelaxationTimes::Int = 0,
    # rng::MersenneTwister = copy(Random.GLOBAL_RNG), 
    rng::Xoshiro = copy(Random.GLOBAL_RNG),
    seed::UInt = rand(Random.RandomDevice(),UInt)
    ) where T<:Lattice

    mc = MonteCarlo(deepcopy(lattice), beta, thermalizationSweeps, measurementSweeps, measurementRate, replicaExchangeRate, reportInterval, checkpointInterval, overrelaxationTimes, rng, seed, 0, Observables(lattice))
...
function run!(mc::MonteCarlo{T}; outfile::Union{String,Nothing}=nothing) where T<:Lattice
    ...
    if mc.sweep == 0
        if mc.lattice.spinType == "Heisenberg"
            for i in 1:length(mc.lattice)
                setSpin!(mc.lattice, i, uniformOnSphere(mc.rng))
            end
        elseif mc.lattice.spinType == "Ising"
            for i in 1:length(mc.lattice)
                setSpin!(mc.lattice, i, randomIsing(mc.rng))
            end
        elseif mc.lattice.spinType == "softIsing"
            for i in 1:length(mc.lattice)
                setSpin!(mc.lattice, i, softIsing(mc.rng))
            end
        elseif mc.lattice.spinType == "mirrorCone"
            for i in 1:length(mc.lattice)
                setSpin!(mc.lattice, i, mirrorCone(mc.rng))
            end
        end
    end
    ...
    while mc.sweep < totalSweeps
        ...
        if mc.overrelaxationTimes != 0
            for idx_or = 1:mc.overrelaxationTimes
                for site in 1:mc.lattice.length
                    ...
                    # field_norm2 = field[1]^2 + field[2]^2
                    # spin_or2D::Vector{Float64} = zeros(2)
                    # spin_or2D[1] = -mc.lattice.spins[1,site] + 2*coef_SH/field_norm2*field[1]
                    # spin_or2D[2] = -mc.lattice.spins[2,site] + 2*coef_SH/field_norm2*field[2]

                    # mc.lattice.spins[1,site] = spin_or2D[1]
                    # mc.lattice.spins[2,site] = spin_or2D[2]
        ...
        for i in 1:length(mc.lattice)
            ...
            elseif mc.lattice.spinType == "Ising"
                newSpinState = tuple(-[getSpin(mc.lattice, site)...]...)
            elseif mc.lattice.spinType == "softIsing"
                newSpinState = softIsing(mc.rng)
            elseif mc.lattice.spinType == "mirrorCone"
                newSpinState = mirrorCone(mc.rng)
            end
        ...
        if mc.sweep >= mc.thermalizationSweeps
            if mc.sweep % mc.measurementRate == 0
                performMeasurements!(mc.observables, mc.lattice, energy, mc.sweep)
            end
        end
