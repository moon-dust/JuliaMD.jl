...
mutable struct Observables
    ...
    structureFactor::LogBinner{Array{Float64, 3},32,BinningAnalysis.Variance{Array{Float64, 3}}}
...
function Observables(lattice::T) where T<:Lattice
    ...
    if dim == 3
        if lattice.calcDyn==true
            ...
            LogBinner(zeros(Float64,lattice.calcLim[1]*lattice.size[1], lattice.calcLim[2]*lattice.size[2], lattice.calcLim[3]*lattice.size[3])),
    ...
    elseif dim == 2
        if lattice.calcDyn==true
            return Observables(ErrorPropagator(Float64), LogBinner(Float64), LogBinner(zeros(Float64,3)), LogBinner(zeros(Float64,lattice.length)), 
            LogBinner(zeros(Float64,lattice.calcLim[1]*lattice.size[1], lattice.calcLim[2]*lattice.size[2], 1)),
    ...
    elseif dim == 1
        if lattice.calcDyn==true
            return Observables(ErrorPropagator(Float64), LogBinner(Float64), LogBinner(zeros(Float64,3)), LogBinner(zeros(Float64,lattice.length)), 
            LogBinner(zeros(Float64, lattice.calcLim[1]*lattice.size[1], 1, 1)),
