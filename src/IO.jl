...
function writeMonteCarlo(filename::String, mc::MonteCarlo{Lattice{D,N}}) where {D,N}
    h5open(filename, "w") do f
        ...
        f["mc/observables/structureFactor/mean"] = mean(mc.observables.structureFactor)
        f["mc/observables/structureFactor/error"] = std_error(mc.observables.structureFactor)
        f["mc/observables/chirality/mean"] = mean(mc.observables.chirality)
        f["mc/observables/chirality/error"] = std_error(mc.observables.chirality)
